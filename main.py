...
from get_frames import (get_rocket_frames,
                        get_garbage_frames,
                        get_game_over_frame)
...
def draw(canvas):
    ...
    rocket = fly_rocket(canvas, max_height//2,
                        max_width//2, rocket_frames)
    ...
    while True:
        coroutines.append(watch_time(canvas, time_game_started))
...
async def watch_time(canvas, time_game_started):
    global year
    watch_window = canvas.derwin(1, 1)
    time_now = time.time()
